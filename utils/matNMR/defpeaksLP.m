...
%
%
%defpeaksLP.m asks the user to mark all peaks in the current 2D spectrum before linear prediction is started
%10-11-2005
...
try
  ...
  delete(findobj(allchild(QmatNMR.TEMPAxis), 'tag', 'LPPeaks'))
  ...
  set(QTEMP1, 'tag', 'LPProjectionDisp');
  ...
  while ((~QmatNMR.Error) & (QmatNMR.ButSoort == 1))
    ...
    if ((~ QmatNMR.Error) & (QmatNMR.ButSoort == 1)) 
      ...
      hold off
      set(QmatNMR.PlotHandle, 'tag', 'LPPeaks');
      ...
      if (QmatNMR.PiekTeller == 2)			%Two positions have been pointed --> peak present !!
        ...
        QmatNMR.LPPeakListIndices = [QmatNMR.LPPeakListIndices QTEMP(1) QTEMP(2)];
        QmatNMR.LPPeakList = [QmatNMR.LPPeakList QTEMP(1):QTEMP(2)];
        if (QTEMPincr > 0)				%ascending axis
          QTEMP2 = QTEMPaxis(QTEMP(1)):0.1:QTEMPaxis(QTEMP(2));	%create a raster area in the current plot
        else
          QTEMP2 = QTEMPaxis(QTEMP(2)):0.1:QTEMPaxis(QTEMP(1));	%create a raster area in the current plot
        end
  
        if length(QTEMP2) > 15
          QTEMP7 = round(length(QTEMP2)/15);
  	if (QTEMPincr > 0)				%ascending axis
            QTEMP2 = QTEMPaxis(QTEMP(1)):QTEMP7:QTEMPaxis(QTEMP(2));
  	else  
            QTEMP2 = QTEMPaxis(QTEMP(2)):QTEMP7:QTEMPaxis(QTEMP(1));
  	end  
        end;  
        ...
        set(QTEMP6, 'linestyle', ':', 'color', 'r', 'tag', 'LPPeaks');
      ...
      if QmatNMR.NumPeaks > 0
        QmatNMR.LPPeakList = sort(QmatNMR.LPPeakList);
      end
