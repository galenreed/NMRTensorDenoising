...
%
%
%regelconvertvarian.m takes care of converting a sequential states experiment into a simultaneous one
...
try
  if (QmatNMR.Dim == 0)
    ...
    if QmatNMR.lbstatus | QmatNMR.bezigmetfase
      disp('matNMR WARNING:  not possible while apodizing or phasing!     regelconvertVarian cancelled');
      ...
      QmatNMR.History = str2mat(QmatNMR.History, 'Converted 2D Varian States FID');
      ...
      QmatNMR.HistoryMacro = AddToMacro(QmatNMR.HistoryMacro, 111);
      ...
      if QmatNMR.RecordingMacro
        ...
        QmatNMR.Macro = AddToMacro(QmatNMR.Macro, 111);
      end
      
      disp('Finished converting current 2D matrix (Varian format) into a matNMR States matrix ...');
    end
  end
