...
%
%
%regelgetFWHH1d tries to find the FWHH of a peak in the current 1D view.
%NOTE: this assumes that other peaks do not interfere
%30-07-2013
...
try
  ...
  %Extract the FWHH
  %
  [Width, Status] = getHumpvalues(QmatNMR.Axis1D(QTEMP3:QTEMP4), real(QmatNMR.Spec1D(QTEMP3:QTEMP4).'), 50);
  if (Status == 1)
    fprintf(1, 'FWHH = %g (in the unit of the current axis)\n', Width);

  else
    disp('Could not determine FWHH with any certainty. Please try again using a better starting condition');
  end
