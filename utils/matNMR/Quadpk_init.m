...
%
%
function Quadpk_init;

% Initialize all the GUI controls for QUADTENSORFIT
...
%
	...
	QuadFig = figure('numbertitle','off',...
	       'Name','QUADTENSORFIT --- Fitting Routine for Quadrupolar Tensors under MAS',...
	       ...
	       'paperposition', [0.1 0.1 .8 .8], ...
               'menubar', 'none', ...
	       'CloseRequestFCN', 'stopquadfit', ...
	       'color', QmatNMR.ColorScheme.Figure1Back, ...
	       'Tag', 'QuadFit');
        if (QmatNMR.unittype == 1)
          set(QuadFig, 'resize', 'on');
        else
          set(QuadFig, 'resize', 'off');
        end
	set(gca, 'visible', 'off');

	%  Uimenubar Object Creation 

	QmatNMR.Q1 = uimenu('parent', QuadFig, 'label', '   Load parameters   ', 'Position', [1]);
		uimenu(QmatNMR.Q1, 'label', 'Load parameters', 'callback', 'quadfit(''loadpars1'');');

	QmatNMR.Q2 = uimenu('parent', QuadFig, 'label', '   View Fit Results   ', 'Position', [2]);
		uimenu(QmatNMR.Q2, 'label', 'Define Results Variable', 'callback', 'quadfit(''defvar1'');');
		uimenu(QmatNMR.Q2, 'label', 'View Fit Results', 'callback', 'quadfit(''view1'');');

  	QmatNMR.Q3 = uimenu(QuadFig, 'label', '   Plot Manipulations   ', 'position', [3]);
	...
	QTEMP1 = uimenu(QuadFig, 'label', 'Macro', 'Position', [4]);
	...
	QmatNMR.Q5 = uimenu('parent', QuadFig, 'label', '   Create Output   ', 'Position', [5]);
		uimenu(QmatNMR.Q5, 'label', 'Printing Menu', 'callback', 'figure(findobj(allchild(0),''tag'',''QuadFit'')); matprint;', 'accelerator', 'p');
		uimenu(QmatNMR.Q5, 'label', 'Save plot on disk', 'callback', 'asksavefitdisk');
  		uimenu(QmatNMR.Q5, 'label', 'Copy Figure', 'callback', 'figure(findobj(allchild(0),''tag'',''QuadFit'')); copyfignmr');

        QTEMP1 = uimenu('parent', QuadFig, 'label', '   HELP ?   ', 'position', [6]);
	...
	QmatNMR.Q6 = uimenu('parent', QuadFig, 'label', '   Clear Functions   ', 'position', [7]);
	...
	%
	set(findobj(QuadFig, 'type', 'uimenu'), 'foregroundcolor', QmatNMR.ColorScheme.UImenuFore);
	...
	stopbut = uicontrol('Style', 'Pushbutton', ...
		...
		'Parent', QuadFig, ...
		'callback', 'askstopquadfit');
	        	
	simbut = uicontrol(... 
		'CallBack','quadfit(''fitbut'');',... 
		'Units', 'Pixels', ...
	...
	simplexbutT = uicontrol('Style', 'PushButton', ...
		'Units', 'Pixels', ...
		'Position', [  5 465 120  35], ...
		'Units', 'Normalized', ...
		'Parent', QuadFig, ...
		'BackGroundColor', QmatNMR.ColorScheme.Button7Back, ...
		'ForeGroundColor', QmatNMR.ColorScheme.Button7Fore, ...
		'String', 'SIMPLEX Quality :');

	simplexbut = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [125 465 100  35], ...
		'Units', 'Normalized', ...
		'String','OFF | 21 | 144 | 233 | 399 | 610 | 987 | 1583 | 2741 | 4409 | 6997 | 11657 | 17389 | 28499 | 43051 | 65063 | 79999 | 2943631',... 
		'Style','Popup',... 
                'Value', 6, ...
		'Parent', QuadFig, ...
		'BackGroundColor', QmatNMR.ColorScheme.Button1Back, ...
	...
	gradbutT = uicontrol('Style', 'PushButton', ...
		'Units', 'Pixels', ...
		'Position', [  5 430 120  35], ...
		'Units', 'Normalized', ...
		'Parent', QuadFig, ...
		'BackGroundColor', QmatNMR.ColorScheme.Button7Back, ...
		'ForeGroundColor', QmatNMR.ColorScheme.Button7Fore, ...
		'String', 'Gradient Quality :');

	gradbut = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [125 430 100  35], ...
		'Units', 'Normalized', ...
		'String','OFF | 21 | 144 | 233 | 399 | 610 | 987 | 1583 | 2741 | 4409 | 6997 | 11657 | 17389 | 28499 | 43051 | 65063 | 79999 | 2943631',... 
		'Style','Popup',... 
		'Parent', QuadFig, ...
	...
	resolutionbutT = uicontrol('Style', 'PushButton', ...
		'Units', 'Pixels', ...
		'Position', [  5 400 120  35], ...
		'Units', 'Normalized', ...
		'Parent', QuadFig, ...
		'BackGroundColor', QmatNMR.ColorScheme.Button7Back, ...
		'ForeGroundColor', QmatNMR.ColorScheme.Button7Fore, ...
		'String', 'Resolution (ppm) :');

	resolutionbut = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [125 400 100  35], ...
		'Units', 'Normalized', ...
		'String','0.05',... 
		'Style','edit',... 
		'Parent', QuadFig, ...
		'BackGroundColor', QmatNMR.ColorScheme.Button1Back, ...
		'ForeGroundColor', QmatNMR.ColorScheme.Button1Fore, ...
		'UserData','resolutionbut');  

%
%Create 2 buttons in the main quadfit window for the carrier frequency
%
		uicontrol('Style', 'PushButton', ...
		'Units', 'Pixels', ...
		'Position', [  5  95  110  35], ...
		'Units', 'Normalized', ...
		'Parent', QuadFig, ...
		'BackGroundColor', QmatNMR.ColorScheme.Button7Back, ...
		'ForeGroundColor', QmatNMR.ColorScheme.Button7Fore, ...
	...
	carrierbut = uicontrol('Style', 'Edit', ...
		'Units', 'Pixels', ...
		'Position', [115  95  80  35], ...
		'Units', 'Normalized', ...
		'Parent', QuadFig, ...
		'BackGroundColor', QmatNMR.ColorScheme.Button1Back, ...
		'ForeGroundColor', QmatNMR.ColorScheme.Button1Fore, ...
...
%
%Create 2 buttons in the main quadfit window for the quantum number
%
		uicontrol('Style', 'PushButton', ...
		'Units', 'Pixels', ...
		'Position', [  5 130 110  35], ...
		'Units', 'Normalized', ...
		'Parent', QuadFig, ...
		'BackGroundColor', QmatNMR.ColorScheme.Button7Back, ...
		'ForeGroundColor', QmatNMR.ColorScheme.Button7Fore, ...
	...
	qnbut = uicontrol('Style', 'Edit', ...
		'Units', 'Pixels', ...
		'Position', [115 130  80  35], ...
		'Units', 'Normalized', ...
		'String','3/2 | 5/2 | 7/2 | 9/2 | 11/2',... 
		'Style','Popup',... 
		'Parent', QuadFig, ...
		'BackGroundColor', QmatNMR.ColorScheme.Button1Back, ...
		'ForeGroundColor', QmatNMR.ColorScheme.Button1Fore, ...
...
%
%Create 3 buttons in the main quadfit window for the background level
%
		uicontrol('Style', 'PushButton', ...
		'Units', 'Pixels', ...
		'Position', [  5  40  90  35], ...
...
%
%Create 3 buttons in the main quadfit window for the slope of the background
%
		uicontrol('Style', 'PushButton', ...
		'Units', 'Pixels', ...
		'Position', [  5   5  90  35], ...
...
%
	  	uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [205 145 106  35], ...
		'Units', 'Normalized', ...
		'Style','PushButton',... 
		'String','Sigma_iso (ppm)',... 
		'Parent', QuadFig, ...
		'BackGroundColor', QmatNMR.ColorScheme.Button7Back, ...
		'ForeGroundColor', QmatNMR.ColorScheme.Button7Fore); 
		...
		'ForeGroundColor', QmatNMR.ColorScheme.Button7Fore); 
	  	...
	  	uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [665 145 106  35], ...
		'Units', 'Normalized', ...
		'Style','PushButton',... 
		'String','Lorentz. LB (ppm)',... 
		'Parent', QuadFig, ...
		'BackGroundColor', QmatNMR.ColorScheme.Button7Back, ...
		'ForeGroundColor', QmatNMR.ColorScheme.Button7Fore); 
...
%
%First create 4 rows of 6 buttons and 6 check boxes in the main quadfit window
%
        for tel=1:4
          %buttons for sigma_iso
	  ck((tel-1)*6+1) = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [205 110-(tel-1)*35  20  35], ...
		'Units', 'Normalized', ...
		'Style','checkbox',... 
		'Visible','on',...
		'Parent', QuadFig, ...
	  ...
	  ed((tel-1)*6+1) = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [225 110-(tel-1)*35  86  35], ...
		'Units', 'Normalized', ...
		'Style','edit',... 
		'Visible','on',... 
		'Parent', QuadFig, ...
	  ...
	  ck((tel-1)*6+2) = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [320 110-(tel-1)*35  20  35], ...
		'Units', 'Normalized', ...
		'Style','checkbox',... 
		'Visible','on',...
		'Parent', QuadFig, ...
	  ...
	  ed((tel-1)*6+2) = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [340 110-(tel-1)*35  86  35], ...
		'Units', 'Normalized', ...
		'Style','edit',... 
		'Visible','on',... 
		'Parent', QuadFig, ...
	  ...
	  ck((tel-1)*6+3) = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [435 110-(tel-1)*35  20  35], ...
		'Units', 'Normalized', ...
		'Style','checkbox',... 
		'Visible','on',...
		'Parent', QuadFig, ...
	  ...
	  ed((tel-1)*6+3) = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [455 110-(tel-1)*35  86  35], ...
		'Units', 'Normalized', ...
		'Style','edit',... 
		'Visible','on',... 
		'Parent', QuadFig, ...
	  ...
	  ck((tel-1)*6+4) = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [550 110-(tel-1)*35  20  35], ...
		'Units', 'Normalized', ...
		'Style','checkbox',... 
                'Value', 1, ...
		'Visible','on',...
	  ...
	  ed((tel-1)*6+4) = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [570 110-(tel-1)*35  86  35], ...
		'Units', 'Normalized', ...
		'Style','edit',... 
		'Visible','on',... 
                'String', '0', ...
	  ...
	  ck((tel-1)*6+5) = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [665 110-(tel-1)*35  20  35], ...
		'Units', 'Normalized', ...
		'Style','checkbox',... 
                'Value', 1, ...
		'Visible','on',...
	  ...
	  ed((tel-1)*6+5) = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [685 110-(tel-1)*35  86  35], ...
		'Units', 'Normalized', ...
		'Style','edit',... 
		'Visible','on',... 
                'String', '0', ...
	  ...
	  ed((tel-1)*6+6) = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [800 110-(tel-1)*35  86  35], ...
		'Units', 'Normalized', ...
		'Style','edit',... 
		'Visible','on',... 
		'Parent', QuadFig, ...
...
%
        handle_list = [fitbut simbut ed(1:26) ck(1:26) TolBut NrIterBut noibut noibute zombut redisbut simplexbut gradbut carrierbut refreshbut printbut stopbut verbbut qnbut resolutionbut];
	...
	%
	set(get(QuadFitAxis, 'title'), 'color', QmatNMR.ColorScheme.AxisFore);

	set(QuadFitAxis, 'units', 'normalized');
        %finally, set the size of the window to the user-defined values in normalized units
	set(QuadFig,'userdata',QTEMP, 'units', 'normalized', ...
