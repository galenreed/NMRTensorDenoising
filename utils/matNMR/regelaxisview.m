...
%
%
%regelaxisview.m sets the view for the current axis
%25-01-'05
...
try
  if QmatNMR.buttonList == 1			%OK-button
    ...
    QTEMP21 = gcf; 	%the current figure window
    				...
    				%First evaluate the values for the view
    ...
    if (QmatNMR.RecordingPlottingMacro)
      ...
      QmatNMR.Macro = AddToMacro(QmatNMR.Macro, 744, QTEMP12.SubPlots, QTEMP1, QTEMP2, QmatNMR.uiInput3);
    end
    ...
    if (QmatNMR.uiInput3 == 1)			%Only change selected axes
      ...
      for QTEMP4 = 1:length(QmatNMR.AllAxes)
        ...
        if ~isempty(QmatNMR.AxesNR)
          set(QmatNMR.AllAxes(QTEMP4), 'view', [QTEMP1 QTEMP2]);
        end	  
      end
      ...
      disp('Axis view changed for selected axes ...');
      if (~QmatNMR.BusyWithMacro)
        drawnow
      end
    ...
    elseif (QmatNMR.uiInput3 == 2) 		%apply to all subplots
      ...
      disp('Axis view changed for all axes ...');
      if (~QmatNMR.BusyWithMacro)
        drawnow
      end
    ...
    elseif (QmatNMR.uiInput3 == 3) 		%apply to the current row of subplots
      ...
      disp('Axis view changed for all axes in the current row ...');
    ...
    elseif (QmatNMR.uiInput3 == 4) 		%apply to the current column of subplots
      ...
      disp('Axis view changed for all axes in the current column ...');
    end  
  
  else
    disp('Changing of axis view was cancelled ...');
  end  
