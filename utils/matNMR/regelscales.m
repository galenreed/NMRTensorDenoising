...
%
%
%regelscales.m sets the xscale, yscale and zscale properties for the current axis
%15-12-'98
...
try
  if QmatNMR.buttonList == 1			%OK-button
    ...
    if (QmatNMR.RecordingPlottingMacro)
      ...
      QmatNMR.Macro = AddToMacro(QmatNMR.Macro, 738, QTEMP12.SubPlots, QmatNMR.uiInput1, QmatNMR.uiInput1a, QmatNMR.uiInput2, QmatNMR.uiInput2a, QmatNMR.uiInput3,  QmatNMR.uiInput3a, QmatNMR.uiInput4);
    end
    ...
    QTEMP = str2mat('linear', 'log');
    QTEMP1 = QTEMP(QmatNMR.uiInput1, :);
    QTEMP2 = QTEMP(QmatNMR.uiInput2, :);
    QTEMP3 = QTEMP(QmatNMR.uiInput3, :);
  
    if (QmatNMR.uiInput4 == 1)			%Only change the current axis
      ...
      for QTEMP4 = 1:length(QmatNMR.AllAxes)
        ...
        if ~isempty(QmatNMR.AxesNR)
          if QmatNMR.uiInput1a
            set(QmatNMR.AllAxes(QTEMP4), 'xscale', QTEMP1);
          end
          if QmatNMR.uiInput2a
            set(QmatNMR.AllAxes(QTEMP4), 'yscale', QTEMP2);
          end
          if QmatNMR.uiInput3a
            set(QmatNMR.AllAxes(QTEMP4), 'zscale', QTEMP3);
          end
        end	  
      end
      ...
      disp('Axis scaling types manually changed for the current axis ...');
    ...
    elseif (QmatNMR.uiInput4 == 2) 		%apply to all subplots
      ...
      disp('Axis scaling types manually changed for all axes ...');
    ...
    elseif (QmatNMR.uiInput4 == 3) 		%apply to the current row of subplots
      ...
      disp('Axis scaling types changed for all axes in the current row ...');
    ...
    elseif (QmatNMR.uiInput4 == 4) 		%apply to the current column of subplots
      ...
      disp('Axis scaling types for all axes in the current column ...');
    end  
  
  else
    disp('Setting of xscale, yscale and zscale properties was cancelled ...');
  end  
