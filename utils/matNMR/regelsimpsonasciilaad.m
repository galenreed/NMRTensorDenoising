...
%
%
%regelsimpsonasciilaad.m reads a SIMPSON ASCII file from disk.
%22-06-'00

try
  if (QmatNMR.buttonList==1)
    watch;
    disp('Please wait while matNMR is reading the SIMPSON ASCII file ...');
  ...
  %
    if (isempty(findstr(QmatNMR.Fname, '$')))
      ...
      eval([QmatNMR.namelast ' = SIMPSONread(QmatNMR.Fname);']);

      %
      %display status output
      %
      fprintf(1, 'The SIMPSON FID/Spectrum was written in the workspace as %s.\n', QmatNMR.namelast);
      Arrowhead;
      ...
      if (QmatNMR.LoadINTOmatNMRDirectly)
        ...
        eval(['QTEMP9 = sort(size(' QmatNMR.namelast '.Spectrum));']);
        ...
        if (QTEMP9(1) == 1)
          ...
          %
          QmatNMR.ask = 1;
          eval(['QTEMP9 = ' QmatNMR.namelast '.FIDstatusTD2;']);
          if (QTEMP9 == 1) 	%spectrum
          ...
          QmatNMR.ask = 2;
          eval(['QTEMP9 = ' QmatNMR.namelast '.FIDstatusTD2;']);
          eval(['QTEMP10= ' QmatNMR.namelast '.FIDstatusTD1;']);
          if (QTEMP9 == 2) 		%FID
            QmatNMR.buttonList = 1;
          elseif (QTEMP10 == 2) 	%spectrum in TD2, FID in TD1
            QmatNMR.buttonList = 3;
          else 			%spectrum
            QmatNMR.buttonList = 2;
          end
          regelnaam
    ...
    disp('Importing of SIMPSON ASCII file cancelled ...');
  end
