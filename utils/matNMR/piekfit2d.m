...
%
%
%piekfit2d.m is the interface between matNMR and the peak fitting routine by Sean M. Brennan
%22-4-'97
...
try
  if (QmatNMR.Dim == 0)
    ...
    if QmatNMR.Dim == 1
      lsq(QmatNMR.Axis1D(QTEMP3:QTEMP4)',(real(QmatNMR.Spec2D(:, QTEMP3:QTEMP4)')));  	%take all rows
    elseif QmatNMR.Dim == 2
      lsq(QmatNMR.Axis1D(QTEMP3:QTEMP4)',(real(QmatNMR.Spec2D(QTEMP3:QTEMP4, :)))); 	%take all columns 	
    end
  end
