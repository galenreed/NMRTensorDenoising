...
global Pverbose QmatNMR

if (x(1,1) < x(2,1))
  plotcmd='plot(x(:,1),y,QmatNMR.LineColor,x(:,1), f, QmatNMR.color(rem(1, length(QmatNMR.color)) + 1)); set(gca, ''XGrid'',''on'', ''YGrid'',''on'', ''XDir'', ''reverse''); axis([min(x) max(x) min(y)-0.1*abs(min(y))-1e-10 max(y)+0.1*abs(max(y))+1e-10]); drawnow';
else
  plotcmd='plot(fliplr(x(:,1)),fliplr(y), QmatNMR.LineColor, fliplr(x(:,1)),fliplr(f), QmatNMR.color(rem(1, length(QmatNMR.color)) + 1)); set(gca, ''XGrid'',''on'', ''YGrid'',''on'', ''XDir'', ''normal''); axis([min(x) max(x) min(y)-0.1*abs(min(y))-1e-10 max(y)+0.1*abs(max(y))+1e-10]); drawnow';
end
...
% jacco, jabe, matNMR
%
% to constrain the percentage of a lorentzian fraction between 0 and 1
%

  for teller=4:4:length(pprev)
    if pprev(teller) > 1
      pprev(teller) = 1;
    else
      if pprev(teller) < 0
...
% jacco, jabe, matNMR
%
% to constrain the width to > 0
%
  for teller=3:4:length(pprev)
    if pprev(teller) < 0
      pprev(teller) = abs(pprev(teller));
    end
  end
...
%        
  ...
  if (mod(IterTel,Pverbose(2)) == 0)
    fprintf(1, '\r     --> peakfit in progress, iteration nr : %g', IterTel);
    eval(plotcmd);
  end
  if ss<eps,
    break;
