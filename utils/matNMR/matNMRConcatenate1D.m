...
%
% Concatenates the 1D matrix <NrOfTimes> times along one of two possible directions:
...
function MatrixOut = matNMRConcatenate1D(MatrixIn, NrOfTimes, Direction)
...
%
  if ((ndims(MatrixIn) == 2) & ~isa(MatrixIn, 'struct'))
    ...
    if ((SizeTD1 ~= 1) & (SizeTD2 ~= 1))
      beep
      disp('matNMRConcatenate1D ERROR: data  is not 1D. Aborting ...');
      return;
