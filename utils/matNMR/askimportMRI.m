...
%
%
%askimportMRI.m opens an input window to get information about the MRI data that needs to be
%read into MATLAB
%27-01-'10

try
  if (QmatNMR.Xfilename == 0)			%first check whether a previous filename exists
    importMRI					%if not, go to the normal mode
    ...
    %
    QuiInput('Read MRI data from disk :', 'OK | CANCEL', 'regelimportMRI', [], ...
             'Name of file : &CAregelinputbuttonsfidlaadSeries4', QmatNMR.Xtext, ...
             'Size in T2 : ', QmatNMR.size1last(1), ...
             'Size in T1 (points) : ', QmatNMR.size2last, ...
             'Name in Workspace : ', QmatNMR.namelast, ...
             '&POFile Format : | SMIS (.mrd) | Siemens RDA (.rda) | Siemens RAW (.raw) | DICOM (.dcm)&CAregelinputbuttonsimportMRI', QmatNMR.DataFormatMRI, ...
             '&CKRead standard parameter files? &CAregelinputbuttonsimportMRI', QmatNMR.ReadParameterFilesFlag, ...
             '&CKLoad into matNMR directly?', QmatNMR.LoadINTOmatNMRDirectly);
  
    regelinputbuttonsimportMRI		%update the buttons according to the selected file format
  end  
