...
%
%
%dispcont.m is the routine that handles the plotting of the contours
%14-12-'96

try
  watch;
  					%Set the flag for relative contour levels
  QmatNMR.ContType = 0;		
  QmatNMR.RestorePeaklist = 0; 	%flag to make sure that plotcont executes the restorepeaklist routine
  calccontlevels
  
  QmatNMR.titelstring2 = ([num2str(QmatNMR.numbcont), ' contours between ', num2str(QmatNMR.under), '% and ', num2str(QmatNMR.over), '% (factor = ' num2str(QmatNMR.multcont) ')']);
  
  if ~isempty(QmatNMR.ContourLevels)
    plotcont
