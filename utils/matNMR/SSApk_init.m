...
% Initialize all the GUI controls for SSATENSORFIT
...
%
	...
	SSAFig = figure('numbertitle','off',...
	       'Name','SSATENSORFIT --- Fitting Routine for MAS sideband intensities',...
	       ...
	       'CloseRequestFCN', 'stopssafit', ...
               'menubar', 'none', ...
	       'color', QmatNMR.ColorScheme.Figure1Back, ...
	       'Tag', 'SSAFit');
        if (QmatNMR.unittype == 1)
          set(SSAFig, 'resize', 'on');
        else
          set(SSAFig, 'resize', 'off');
        end
	set(gca, 'visible', 'off');

	%  Uimenubar Object Creation 

	QmatNMR.Q1 = uimenu('parent', SSAFig, 'label', '   Load parameters   ', 'Position', [1]);
		uimenu(QmatNMR.Q1, 'label', 'Load parameters', 'callback', 'ssafit(''loadpars1'');');

	QmatNMR.Q2 = uimenu('parent', SSAFig, 'label', '   View Fit Results   ', 'Position', [2]);
		uimenu(QmatNMR.Q2, 'label', 'Define Results Variable', 'callback', 'ssafit(''defvar1'');');
		uimenu(QmatNMR.Q2, 'label', 'View Fit Results', 'callback', 'ssafit(''view1'');');

  	QmatNMR.Q3 = uimenu(SSAFig, 'label', '   Plot Manipulations   ', 'position', [3]);
	...
	QTEMP1 = uimenu(SSAFig, 'label', 'Macro', 'Position', [4]);
	...
	QmatNMR.Q5 = uimenu('parent', SSAFig, 'label', '   Create Output   ', 'Position', [5]);
		uimenu(QmatNMR.Q5, 'label', 'Printing Menu', 'callback', 'figure(findobj(allchild(0),''tag'',''SSAFit'')); matprint;', 'accelerator', 'p');
		uimenu(QmatNMR.Q5, 'label', 'Save plot on disk', 'callback', 'asksavefitdisk');
  		uimenu(QmatNMR.Q5, 'label', 'Copy Figure', 'callback', 'figure(findobj(allchild(0),''tag'',''SSAFit'')); copyfignmr');

        QTEMP1 = uimenu('parent', SSAFig, 'label', '   HELP ?   ', 'position', [6]);
	...
	QmatNMR.Q6 = uimenu('parent', SSAFig, 'label', '   Clear Functions   ', 'position', [7]);
	...
	%
	set(findobj(SSAFig, 'type', 'uimenu'), 'foregroundcolor', QmatNMR.ColorScheme.UImenuFore);
	...
	refreshbut = uicontrol(... 
		'CallBack','ssafit(''refreshbut'');',... 
		'Units', 'Pixels', ...
		'Position', [  5 265 110  50], ...
		'Units', 'Normalized', ...
		'String','Refresh',... 
	...
	gammabutT = uicontrol('Style', 'PushButton', ...
		'Units', 'Pixels', ...
		'Position', [  5 395 120  35], ...
		'Units', 'Normalized', ...
		'Parent', SSAFig, ...
		'BackGroundColor', QmatNMR.ColorScheme.Button7Back, ...
		'ForeGroundColor', QmatNMR.ColorScheme.Button7Fore, ...
...
%
%Create 2 buttons in the main ssafit window for the carrier frequency
%
		uicontrol('Style', 'PushButton', ...
		'Units', 'Pixels', ...
		'Position', [  5  40  150  35], ...
		'Units', 'Normalized', ...
		'Parent', SSAFig, ...
		'BackGroundColor', QmatNMR.ColorScheme.Button7Back, ...
		'ForeGroundColor', QmatNMR.ColorScheme.Button7Fore, ...
...
%
%Create 2 buttons in the main ssafit window for the spinning speed
%
		uicontrol('Style', 'PushButton', ...
		'Units', 'Pixels', ...
		'Position', [  5   5 150  35], ...
		...
		'BackGroundColor', QmatNMR.ColorScheme.Button7Back, ...
		'ForeGroundColor', QmatNMR.ColorScheme.Button7Fore, ...
		'String', 'Spinning speed (kHz) :');

	omegarbut = uicontrol('Style', 'Edit', ...
		'Units', 'Pixels', ...
		'Position', [155   5 120  35], ...
		'Units', 'Normalized', ...
		'String','1',... 
		'Style','Edit',... 
		'Parent', SSAFig, ...
...
%
%First create 2 rows of 3 buttons and 6 check boxes in the main ssafit window
%
          tel = 1;
          %buttons for sigma_iso
	  ed((tel-1)*4+1) = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [320 110-(tel-1)*105 120  35], ...
		'Units', 'Normalized', ...
		'Style','edit',... 
		'Visible','on',... 
                'String', '0', ...
	  ...
	  ck((tel-1)*4+1) = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [470 110-(tel-1)*105  20  35], ...
		'Units', 'Normalized', ...
		'Style','checkbox',... 
		'Visible','on',...
		'Parent', SSAFig, ...
	  ...
	  ed((tel-1)*4+2) = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [490 110-(tel-1)*105 100  35], ...
		'Units', 'Normalized', ...
		'Style','edit',... 
		'Visible','on',... 
                'String', '0', ...
	  ...
	  ck((tel-1)*4+2) = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [620 110-(tel-1)*105  20  35], ...
		'Units', 'Normalized', ...
		'Style','checkbox',... 
		'Visible','on',...
		'Parent', SSAFig, ...
	  ...
	  ed((tel-1)*4+3) = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [640 110-(tel-1)*105 100  35], ...
		'Units', 'Normalized', ...
		'Style','edit',... 
                'String', '0', ...
		'Visible','on',... 
	  ...
	  ck((tel-1)*4+3) = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [760 110-(tel-1)*105  20  35], ...
		'Units', 'Normalized', ...
		'Style','checkbox',... 
		'Visible','on',...
		'Parent', SSAFig, ...
		...
		'ForeGroundColor', QmatNMR.ColorScheme.Button1Fore, ...
		'UserData',['ck(' num2str((tel-1)*4+2) ')']);
	  ed((tel-1)*4+4) = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [780 110-(tel-1)*105 100  35], ...
		'Units', 'Normalized', ...
		'Style','edit',... 
                'String', '0', ...
		'Visible','on',... 
		...
		'callback',['ssafit(''ed'')']);
          ...
          %
          %row 2 = the corresponding tensor components sigma11, sigma22 and sigma33
          %
          tel = 2;
          %buttons for sigma_11
	  ed((tel-1)*4+1) = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [395 110-(tel-1)*105 120  35], ...
		'Units', 'Normalized', ...
		'Style','Pushbutton',... 
		'Visible','on',... 
                'String', '0', ...
	  ...
	  ed((tel-1)*4+2) = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [545 110-(tel-1)*105 120  35], ...
		'Units', 'Normalized', ...
		'Style','Pushbutton',... 
		'Visible','on',... 
                'String', '0', ...
	  ...
	  ed((tel-1)*4+3) = uicontrol(... 
		'Units', 'Pixels', ...
		'Position', [695 110-(tel-1)*105 120  35], ...
		'Units', 'Normalized', ...
		'Style','Pushbutton',... 
                'String', '0', ...
		'Visible','on',... 
...
%
        handle_list = [fitbut simbut ed ck TolBut NrIterBut noibut noibute zombut redisbut simplexbut gradbut gammabut carrierbut omegarbut refreshbut printbut stopbut verbbut];
	...
	%
	set(get(SSAFitAxis, 'title'), 'color', QmatNMR.ColorScheme.AxisFore);

	set(SSAFitAxis, 'units', 'normalized');
        %finally, set the size of the window to the user-defined values in normalized units
	set(SSAFig,'userdata',QTEMP, 'units', 'normalized', ...
