...
%
%
%regelaxiscolors.m sets the xcolor, ycolor and zcolor properties for the current axis
%31-03-'99
...
try
  if QmatNMR.buttonList == 1			%OK-button
    ...
    QTEMP1 = deblank(fliplr(deblank(fliplr(QmatNMR.uiInput1))));
    if strcmp(QTEMP1, '')
      QTEMP1 = [];
    else  
      if (~strcmp(QTEMP1(1), '['))
      ...
      QTEMP1 = eval([QTEMP5 QTEMP1 QTEMP6]);
    end  
    
    QTEMP2 = deblank(fliplr(deblank(fliplr(QmatNMR.uiInput2))));
    if strcmp(QTEMP2, '')
      QTEMP2 = [];
    else  
      if (~strcmp(QTEMP2(1), '['))
      ...
      QTEMP2 = eval([QTEMP5 QTEMP2 QTEMP6]);
    end  
    
    QTEMP3 = deblank(fliplr(deblank(fliplr(QmatNMR.uiInput3))));
    if strcmp(QTEMP3, '')
      QTEMP3 = [];
    else  
      if (~strcmp(QTEMP3(1), '['))
      ...
      QTEMP3 = eval([QTEMP5 QTEMP3 QTEMP6]);
    end  
    ...
    if (QmatNMR.RecordingPlottingMacro)
      ...
      QmatNMR.Macro = AddToMacro(QmatNMR.Macro, 723, QTEMP12.SubPlots, QmatNMR.uiInput1a, QmatNMR.uiInput2a, QmatNMR.uiInput3a, QmatNMR.uiInput4);
    end
    ...
    if (QmatNMR.uiInput4 == 1)			%Only change selected axes
      ...
      for QTEMP4 = 1:length(QmatNMR.AllAxes)
        ...
        if ~isempty(QmatNMR.AxesNR)
          if QmatNMR.uiInput1a
            set(QmatNMR.AllAxes(QTEMP4), 'xcolor', QTEMP1);
  	end
          if QmatNMR.uiInput2a
            set(QmatNMR.AllAxes(QTEMP4), 'ycolor', QTEMP2);
  	end
          if QmatNMR.uiInput3a
            set(QmatNMR.AllAxes(QTEMP4), 'zcolor', QTEMP3);
  	end
        end	  
      end
      ...
      disp('Axis colors changed for selected axes ...');
      if (~QmatNMR.BusyWithMacro)
        drawnow
      end
    ...
    elseif (QmatNMR.uiInput4 == 2) 		%apply to all subplots
      ...
      disp('Axis colors changed for all axes ...');
      if (~QmatNMR.BusyWithMacro)
        drawnow
      end
    ...
    elseif (QmatNMR.uiInput4 == 3) 		%apply to the current row of subplots
      ...
      disp('Axis colors changed for all axes in the current row ...');
    ...
    elseif (QmatNMR.uiInput4 == 4) 		%apply to the current column of subplots
      ...
      disp('Axis colors changed for all axes in the current column ...');
    end  
  
  else
    disp('Setting of axis colors was cancelled ...');
  end  
