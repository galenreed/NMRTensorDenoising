...
T1leasqr(x,y,pin,func,stol,niter,wt,dp,dfdp,options)
...
plotcmd='plot(x, y, [QmatNMR.LineColor ''p''], x, f, QmatNMR.color(rem(1, length(QmatNMR.color)) + 1));';

vernum= sscanf(version,'%f');
if vernum(1) >= 4,
  global Pverbose
  plotcmd='QTEMP = axis; plot(x,y,[QmatNMR.LineColor ''p''],x,f, QmatNMR.color(rem(1, length(QmatNMR.color)) + 1)); set(gca, ''XGrid'',''on'', ''YGrid'',''on''); axis(QTEMP); drawnow';
end
...
if Pverbose(3)
  ...
  [p opts] = matNMRsimplex('T1simplex', p, options, [], x, y, p, dp);	
