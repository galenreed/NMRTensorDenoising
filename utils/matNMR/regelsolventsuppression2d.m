...
%
%
%regelsolventsuppression2d.m performs the solvent suppression routine as described
...
try
  if QmatNMR.buttonList == 1						%= OK-button
    ...
    %
    %execute the solvent deconvolution
    %
    if (QmatNMR.Dim == 1)				%TD 2
      ...
      if (QmatNMR.SolventSuppressionWindow == 1) 		%use gaussian window function
        ...
        QmatNMR.History = str2mat(QmatNMR.History, ['Solvent suppression performed on TD 2 with Gaussian, width=' num2str(QmatNMR.SolventSuppressionWidth) ', jump size extrapolation=' num2str(QmatNMR.SolventSuppressionExtrapolate)]);
      ...
      elseif (QmatNMR.SolventSuppressionWindow == 2) 	%use sine-bell function
        ...
        QmatNMR.History = str2mat(QmatNMR.History, ['Solvent suppression performed on TD 2 with Sine-Bell, width=' num2str(QmatNMR.SolventSuppressionWidth) ', jump size extrapolation=' num2str(QmatNMR.SolventSuppressionExtrapolate)]);
        ...
        QmatNMR.History = str2mat(QmatNMR.History, ['Solvent suppression performed on TD 2 with Rectangle, width=' num2str(QmatNMR.SolventSuppressionWidth) ', jump size extrapolation=' num2str(QmatNMR.SolventSuppressionExtrapolate)]);
      end
      ...
      if ~((QmatNMR.howFT == 3) | (QmatNMR.howFT == 6))	%anything but States or States-TPPI
        ...
        %
        QmatNMR.Spec2D = (QmatNMR.Spec2D - (QTEMP1 + Qi*QTEMP4)/QmatNMR.NormalizationFactor);
        ...
        %
        QmatNMR.Spec2D = (QmatNMR.Spec2D - (QTEMP1 + Qi*QTEMP4)/QmatNMR.NormalizationFactor);
        QmatNMR.Spec2Dhc = (QmatNMR.Spec2Dhc - (QTEMP6 + Qi*QTEMP7)/QmatNMR.NormalizationFactor);
      end
      ...
      if (QmatNMR.SolventSuppressionWindow == 1) 		%use gaussian window function
        ...
        QmatNMR.History = str2mat(QmatNMR.History, ['Solvent suppression performed on TD 1 with Gaussian, width=' num2str(QmatNMR.SolventSuppressionWidth) ', jump size extrapolation=' num2str(QmatNMR.SolventSuppressionExtrapolate)]);
      ...
      elseif (QmatNMR.SolventSuppressionWindow == 2) 	%use sine-bell function
        ...
        QmatNMR.History = str2mat(QmatNMR.History, ['Solvent suppression performed on TD 1 with Sine-Bell, width=' num2str(QmatNMR.SolventSuppressionWidth) ', jump size extrapolation=' num2str(QmatNMR.SolventSuppressionExtrapolate)]);
        ...
        QmatNMR.History = str2mat(QmatNMR.History, ['Solvent suppression performed on TD 1 with Rectangle, width=' num2str(QmatNMR.SolventSuppressionWidth) ', jump size extrapolation=' num2str(QmatNMR.SolventSuppressionExtrapolate)]);
      end
      ...
      if ~((QmatNMR.howFT == 3) | (QmatNMR.howFT == 6))	%anything but States or States-TPPI
        ...
        %
        %And we substract the matrix from the original matrix AND we transpose the matrix again
        %
        QmatNMR.Spec2D = (QmatNMR.Spec2D - (QTEMP1 + Qi*QTEMP4)/QmatNMR.NormalizationFactor).';
    ...
    QmatNMR.HistoryMacro = AddToMacro(QmatNMR.HistoryMacro, 123, QmatNMR.SolventSuppressionWindow, QmatNMR.SolventSuppressionWidth, QmatNMR.SolventSuppressionExtrapolate, QmatNMR.Dim, QmatNMR.howFT);	%code for solvent suppression 2D
    ...
    if QmatNMR.RecordingMacro
      ...
      QmatNMR.Macro = AddToMacro(QmatNMR.HistoryMacro, 123, QmatNMR.SolventSuppressionWindow, QmatNMR.SolventSuppressionWidth, QmatNMR.SolventSuppressionExtrapolate, QmatNMR.Dim, QmatNMR.howFT);	%code for solvent suppression 2D
    end
    ...
    disp(['2D solvent suppression performed on ' QTEMP(QmatNMR.Dim, :)]);
  
  else
    disp('2D solvent suppression cancelled ...');
  end  
