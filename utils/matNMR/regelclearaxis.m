...
%
%
%regelclearaxis.m clears the contents of axes
%31-03-'99
...
try
  if QmatNMR.buttonList == 1			%OK-button
    ...
    if (QmatNMR.RecordingPlottingMacro)
      ...
      QmatNMR.Macro = AddToMacro(QmatNMR.Macro, 722, QTEMP12.SubPlots, QmatNMR.uiInput1, get(QTEMP20, 'userdata'));
    end
    ...
    if (QmatNMR.uiInput1 == 1)			%Only change selected axes
  	...
  	%axes(QmatNMR.AllAxes(QTEMP4));
          set(QTEMP21, 'currentaxes', QmatNMR.AllAxes(QTEMP4));
      ...
      disp('Axis cleared for selected axes ...');
      if (~QmatNMR.BusyWithMacro)
        drawnow
      end
  
  
    elseif (QmatNMR.uiInput1 == 2)	%change all axes in the current plot to the same color axis
      ...
      disp('Axis cleared for all axes ...');
      if (~QmatNMR.BusyWithMacro)
        drawnow
      end
  
  
    elseif (QmatNMR.uiInput1 == 3)	%change all axes in the current row of subplots
      QmatNMR.AllAxes = findobj(findobj(QTEMP21, 'type', 'axes', 'tag', 'RegularAxis'), 'selected', 'on');
      if isempty(QmatNMR.AllAxes)
        QmatNMR.AllAxes = QTEMP20;
      end
      ...
      disp('Axis cleared for all axes in the current row ...');
      if (~QmatNMR.BusyWithMacro)
        drawnow
      end
  
  
    elseif (QmatNMR.uiInput1 == 4)	%change all axes in the current column of subplots
      QmatNMR.AllAxes = findobj(findobj(QTEMP21, 'type', 'axes', 'tag', 'RegularAxis'), 'selected', 'on');
      if isempty(QmatNMR.AllAxes)
        QmatNMR.AllAxes = QTEMP20;
      end
      ...
      disp('Axis cleared for all axes in the current column ...');
      if (~QmatNMR.BusyWithMacro)
        drawnow
      end
    end  
    ...
    disp('Clearing of axes was cancelled ...');
  end  
