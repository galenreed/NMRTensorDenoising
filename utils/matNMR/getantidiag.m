...
%
%
%getantidiag.m takes the anti-diagonal of a 2D variable and shows it as a 1D spectrum
%04-03-'99
...
try
  if (QmatNMR.Dim == 0)
    ...
    if QmatNMR.lbstatus | QmatNMR.bezigmetfase
      disp('matNMR WARNING:  not possible while apodizing or phasing!     getantidiag cancelled');
      ...
      if ((QmatNMR.SizeTD2 == QmatNMR.SizeTD1) & (QmatNMR.Rincr2 == QmatNMR.Rincr1) & (QmatNMR.Rnull2 == QmatNMR.Rnull1))
        QmatNMR.Spec1D = diag(flipud(QmatNMR.Spec2D)).';
        ...
        %Now interpolate
        %
        QmatNMR.Spec1D = interp2(QmatNMR.AxisTD2, QmatNMR.AxisTD1, QmatNMR.Spec2D, QTEMP6, fliplr(QTEMP6), 'linear');
      ...
      QmatNMR.RulerXAxis = 1;		%we have defined a user-defined axis in points
  
      if (~QmatNMR.BusyWithMacro)
        %clear the previous 1D undo matrix
        QmatNMR.UnDoMatrix1D = GenerateMatNMRStructure;
        asaanpas;
        title(['The anti-diagonal from ' strrep(QmatNMR.Spec2DName, '_', '\_')], 'Color', QmatNMR.ColorScheme.AxisFore);
      end  
    
    %add this action to the processing history
      QmatNMR.History = str2mat(QmatNMR.History, 'Switched from 2D to 1D mode');
      QmatNMR.History = str2mat(QmatNMR.History, 'Extracted the anti-diagonal from the spectrum');
      QmatNMR.HistoryMacro = AddToMacro(QmatNMR.HistoryMacro, 202);	%code for anti-diagonal
      if QmatNMR.RecordingMacro
        QmatNMR.Macro = AddToMacro(QmatNMR.HistoryMacro, 202);	%code for anti-diagonal
      end
    
      disp('Anti-Diagonal finished');
      Arrowhead;
    end
  end
