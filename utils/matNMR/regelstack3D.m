...
%
%
%regelstack3D.m displays a stack 3D plot from the current 2D matrix in matNMR
%04-01-'00

try
  if QmatNMR.buttonList == 1
    [QmatNMR.SpecName2D3D QmatNMR.CheckInput] = checkinput(QmatNMR.uiInput1);
    QmatNMR.Stack3DDimension = abs(eval(QmatNMR.uiInput2));
    QmatNMR.stackaz = abs(eval(QmatNMR.uiInput3));
    QmatNMR.stackel = abs(eval(QmatNMR.uiInput4));
    QmatNMR.UserDefAxisT2Cont = QmatNMR.uiInput5;
    QmatNMR.UserDefAxisT1Cont = QmatNMR.uiInput6;
    QmatNMR.Q2D3DMacro = QmatNMR.uiInput7;
