...
%
%
% matNMRRunningAverage1D
%
% syntax: MatrixOut = matNMRRunningAverage1D(MatrixIn, WindowFunction, WindowWidth, JumpSize, ReplaceFlag)
%
% Performs a running average over the spectrum, which is the inverse of performing a
% solvent deconvolution here. It therefore follows the description given in 
...
%
% ReplaceFlag is used to define whether the points that are principally lost (on the edges)
% are replaced by NaN (default), or by the original values (ReplaceFlag = 1)
%
% Jacco van Beek
% 25-07-2004
%

function MatrixOut = matNMRRunningAverage1D(MatrixIn, WindowFunction, WindowWidth, JumpSize, ReplaceFlag)
...
%
  if ((ndims(MatrixIn) == 2) & ~isa(MatrixIn, 'struct'))
    ...
    if ((SizeTD1 ~= 1) & (SizeTD2 ~= 1))
      beep
      disp('matNMRRunningAverage1D ERROR: data  is not 1D. Aborting ...');
      return;
  ...
  %
  %make sure that the data is a row vector
  %
  MatrixIn = MatrixIn(:).';
...
%
  ...
  %
  MatrixOut = (QTEMP1 + sqrt(-1)*QTEMP4)/QmatNMR.NormalizationFactor;
  
  
  %
  %If wanted, replace the NaN on the edges by the original (unfiltered) values
  %
  if (ReplaceFlag == 1)
    MatrixOut(isinf(MatrixOut)) = MatrixIn(isinf(MatrixOut));
  end