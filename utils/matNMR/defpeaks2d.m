...
%
%
%defpeaks2d.m asks the user to mark all peaks in the current 2D spectrum before the baseline correction
%routine can start.
%20-1-'98
...
try
  ...
  delete(findobj(allchild(gca), 'tag', 'BaselinePeaks'))
  
  
  %
  % Show projection when baseline correcting 2D spectra
  %
  if (get(QmatNMR.Q2dbas13, 'value'))
    				%
    ...
    if (~isempty(get(QmatNMR.Q2dbas11, 'String')))		%starting row/column
      ...
      QTEMP9 = QTEMP8(length(QTEMP8));
      if ((QTEMP9 =='k') | (QTEMP9 == 'K'))
    ...
    if (QmatNMR.Dim < 2)				%covers TD2 and 1D projection modes
      QTEMP9 = QmatNMR.SizeTD1;
    else						%TD1
  
      QTEMP9 = QmatNMR.SizeTD2;
    end  
    if (QmatNMR.SelectedArea(2) > QTEMP9)
      QmatNMR.SelectedArea(2) = QTEMP9;
    end
  
    %
    %Now create the projection and plot it
    %
    if QmatNMR.Dim == 1
      QmatNMR.dual = Qview0(QmatNMR.Spec2D, QmatNMR.SelectedArea(1), QmatNMR.SelectedArea(2));
    else
      QmatNMR.dual = Qview90(QmatNMR.Spec2D, QmatNMR.SelectedArea(1), QmatNMR.SelectedArea(2));
    end
  
    %
    %scale the projection to fit within the current axis limits
    %
    QTEMP1 = max(real(QmatNMR.dual)) - min(real(QmatNMR.dual));
    QmatNMR.dual = (QmatNMR.dual * abs(QmatNMR.totaalY / QTEMP1));
    QmatNMR.dual = QmatNMR.dual - min(real(QmatNMR.dual)) + QmatNMR.ymin;
  
    QmatNMR.dualaxis = QmatNMR.Axis1D;			%now define the axis vector needed for the plot command
    CheckAxisDual;
  
    hold on
    eval(QmatNMR.DualPlotCommand);  
    hold off
    QTEMP1 = findobj(gca, 'type', 'line');		%find all line handles
    set(QTEMP1(1), 'tag', 'BaselineDualDisp');	%the first one, ie the last plotted or current one, is 
  end
  ...
  while ((~QmatNMR.Error) & (QmatNMR.ButSoort == 1))
    ...
    if ((~ QmatNMR.Error) & (QmatNMR.ButSoort == 1)) 
      ...
      %
      % Delete projection when baseline correcting 2D spectra
      %
      if (get(QmatNMR.Q2dbas13, 'value'))
        delete(findobj(allchild(gca), 'tag', 'BaselineDualDisp'));	%the tag has been defined above
      end
      ...
      if (~isempty(QTEMPuicontextFlag))	%Switch context menu on if it existed before
        set(gca, 'uicontextmenu', QTEMPuicontextFlag);
      end
