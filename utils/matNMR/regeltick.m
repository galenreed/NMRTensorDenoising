...
%
%
%regeltick.m sets the xtick, ytick and ztick properties for the current axis
%26-03-'99
...
try
  if (QmatNMR.buttonList == 1)			%OK-button
    ...
    if (QmatNMR.RecordingPlottingMacro)
      ...
      QmatNMR.Macro = AddToMacro(QmatNMR.Macro, 743, QTEMP12.SubPlots, QmatNMR.uiInput1a, QmatNMR.uiInput2a, QmatNMR.uiInput3a, QmatNMR.uiInput4);
    end
    ...
    %Perform the action
    %
    				%First make sure the array has square brackets around itself
    if strcmp(QTEMP1, '')
      QTEMP1 = [];
    else  
      if (~strcmp(QTEMP1(1), '['))
      ...
      if (~strcmp(QTEMP1(length(QTEMP1)), ']'))
        QTEMP6 = ']';
      else
        QTEMP6 = '';
      end    
      QTEMP1 = sort(eval([QTEMP5 QTEMP1 QTEMP6]));
    end  
    ...
    if strcmp(QTEMP2, '')
      ...
      if (~strcmp(QTEMP2(length(QTEMP2)), ']'))
        QTEMP6 = ']';
      else
        QTEMP6 = '';
      end    
      QTEMP2 = sort(eval([QTEMP5 QTEMP2 QTEMP6]));
    end  
    ...
    if strcmp(QTEMP3, '')
      ...
      if (~strcmp(QTEMP3(length(QTEMP3)), ']'))
        QTEMP6 = ']';
      else
        QTEMP6 = '';
      end    
      QTEMP3 = sort(eval([QTEMP5 QTEMP3 QTEMP6]));
    end  
    
  
    %
    %Change the tick positions
    %
    if (QmatNMR.uiInput4 == 1)			%Only change selected axes
      ...
      for QTEMP4 = 1:length(QmatNMR.AllAxes)
        ...
        if ~isempty(QmatNMR.AxesNR)
          if QmatNMR.uiInput1a
            set(QmatNMR.AllAxes(QTEMP4), 'xtick', QTEMP1);
  	end
          if QmatNMR.uiInput2a
            set(QmatNMR.AllAxes(QTEMP4), 'ytick', QTEMP2);
  	end
          if QmatNMR.uiInput3a
            set(QmatNMR.AllAxes(QTEMP4), 'ztick', QTEMP3);
  	end
        end	  
      end
      ...
      disp('Tick positions changed for selected axes ...');
    ...
    elseif (QmatNMR.uiInput4 == 2) 		%apply to all subplots
      ...
      disp('Tick positions changed for all axes ...');
    ...
    elseif (QmatNMR.uiInput4 == 3) 		%apply to the current row of subplots
      ...
      disp('Tick positions changed for all axes in the current row ...');
    ...
    elseif (QmatNMR.uiInput4 == 4) 		%apply to the current column of subplots
      ...
      disp('Tick positions changed for all axes in the current column ...');
    end  
  
  
  elseif (QmatNMR.buttonList == 2) 	%set the tick mode to auto
    QTEMP20 = gca;
    QTEMP21 = gcf; 	%the current figure window
    ...
    if (QmatNMR.uiInput4 == 1)			%Only change selected axes
      ...
      for QTEMP4 = 1:length(QmatNMR.AllAxes)
        ...
        if ~isempty(QmatNMR.AxesNR)
          if QmatNMR.uiInput1a
            set(QmatNMR.AllAxes(QTEMP4), 'xtickmode', 'auto');
  	end
          if QmatNMR.uiInput2a
            set(QmatNMR.AllAxes(QTEMP4), 'ytickmode', 'auto');
  	end
          if QmatNMR.uiInput3a
            set(QmatNMR.AllAxes(QTEMP4), 'ztickmode', 'auto');
  	end
        end	  
      end
    ...
    disp('Changing of tick positions cancelled ...');
  end  
