...
%
%
%regelhold.m handles changing whether or not to turn on the hold property in the axis
%13-07-'99
...
try
  if QmatNMR.buttonList == 1		%OK-button
    ...
    if (QmatNMR.RecordingPlottingMacro)
      ...
      QmatNMR.Macro = AddToMacro(QmatNMR.Macro, 732, QTEMP12.SubPlots, QmatNMR.uiInput1, QmatNMR.uiInput2);
    end
    ...
    %Perform the action
    %
    QTEMP4 = str2mat('off', 'on');
    QTEMP5 = str2mat('replacechildren', 'add');
    ...
    if (QmatNMR.uiInput2 == 1)		%only change selected axes
      ...
      for QTEMP2 = 1:length(QmatNMR.AllAxes)
        ...
        if ~isempty(QmatNMR.AxesNR)
          set(QmatNMR.AllAxes(QTEMP2), 'nextplot', QTEMP5(QmatNMR.uiInput1, :));
        end	  
      end
      ...
      disp(['Hold set to ' QTEMP1 ' for selected axes ...']);
    ...
    elseif (QmatNMR.uiInput2 == 2)		%change all axes in the current plot
      ...
      disp(['Hold property changed to ' QTEMP1 ' for all axes ...']);
    ...
    elseif (QmatNMR.uiInput2 == 3)		%apply to the current row of subplots
      ...
      disp(['Hold property changed to ' QTEMP1 ' for all axes in the current row ...']);
    ...
    elseif (QmatNMR.uiInput2 == 4)		%apply to the current column of subplots
      ...
      disp(['Hold property changed to ' QTEMP1 ' for all axes in the current column ...']);
    end
  
  else
    disp('Hold property not changed ...');
  end
