...
if (fp == -1)
  disp(['matNMR ERROR: couldn''t open parameter file "' FileName '". Aborting ...']);
  message
  Parameters = [];
  return
end
...
%
Parameters = fgetl(fp);
Parameters([findstr(Parameters, '#') findstr(Parameters, '$') findstr(Parameters, ';')]) = '';
...
while 1
  ...
  if ischar(tmp)
    ...
    if (length(tmp)<80)		%avoid endless lines and thus and memory time loss
      ...
      Parameters = str2mat(Parameters, tmp);
    end
  else
    break
  end
