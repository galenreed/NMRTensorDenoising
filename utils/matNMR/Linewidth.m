...
%
%
%Linewidth.m sets the linewidth in the current plot to a certain value ...
%
%11-06-'98

function ret = Linewidth(Width);
  ...
  if (QmatNMR.RecordingPlottingMacro)
    ...
    QmatNMR.Macro = AddToMacro(QmatNMR.Macro, 734, QTEMP12.SubPlots, Width, get(QTEMP20, 'userdata'));
  end
  ...
  set(QmatNMR.AllAxes, 'linewidth', Width);
  QTEMP = get(QmatNMR.AllAxes, 'children');
  if iscell(QTEMP)
    QTEMP = cell2mat(QTEMP);
  end
  set(findobj(QTEMP, 'type', 'line'), 'linewidth', Width);
  set(findobj(QTEMP, 'type', 'patch'), 'linewidth', Width);
  set(findobj(QTEMP, 'type', 'surface'), 'linewidth', Width);
