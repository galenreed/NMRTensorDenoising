...
%
%
%regelrunningav1d.m performs the running average routine as described
...
try
  if QmatNMR.buttonList == 1						%= OK-button
    ...
    if (QmatNMR.SolventSuppressionWindow == 1) 		%use gaussian window function
      ...
      QmatNMR.History = str2mat(QmatNMR.History, ['Running average performed with Gaussian, width=' num2str(QmatNMR.SolventSuppressionWidth) ', extrapolate=' num2str(QmatNMR.SolventSuppressionExtrapolate)]);
    ...
    elseif (QmatNMR.SolventSuppressionWindow == 2) 	%use sine-bell function
      ...
      QmatNMR.History = str2mat(QmatNMR.History, ['Running average performed with Sine-Bell, width=' num2str(QmatNMR.SolventSuppressionWidth) ', extrapolate=' num2str(QmatNMR.SolventSuppressionExtrapolate)]);
      ...
      QmatNMR.History = str2mat(QmatNMR.History, ['Running average performed with Rectangle, width=' num2str(QmatNMR.SolventSuppressionWidth) ', extrapolate=' num2str(QmatNMR.SolventSuppressionExtrapolate)]);
    end
    ...
    %
    %keep the low-frequency approximation from the FID
    %
    QmatNMR.Spec1D = (QTEMP1 + sqrt(-1)*QTEMP4)/QmatNMR.NormalizationFactor;
    ...
    QmatNMR.HistoryMacro = AddToMacro(QmatNMR.HistoryMacro, 29, QmatNMR.SolventSuppressionWindow, QmatNMR.SolventSuppressionWidth, QmatNMR.SolventSuppressionExtrapolate);	%code for solvent suppression 1D
    ...
    if QmatNMR.RecordingMacro
      ...
      QmatNMR.Macro = AddToMacro(QmatNMR.HistoryMacro, 29, QmatNMR.SolventSuppressionWindow, QmatNMR.SolventSuppressionWidth, QmatNMR.SolventSuppressionExtrapolate);	%code for solvent suppression 1D
    end
    ...
    disp('1D running average performed');
  
  else
    disp('1D running average cancelled ...');
