...
%
%
%regelaxislabels.m sets the XLabel, YLabel and ZLabel properties for the current axis
%08-04-'99
...
try
  if QmatNMR.buttonList == 1			%OK-button
    ...
    if (QmatNMR.RecordingPlottingMacro)
      ...
      QmatNMR.Macro = AddToMacro(QmatNMR.Macro, 725, QTEMP12.SubPlots, QmatNMR.uiInput1a, QmatNMR.uiInput2a, QmatNMR.uiInput3a, QmatNMR.uiInput4);
    end
    ...
    %Perform the action
    %
    if (QmatNMR.uiInput4 == 1)			%Only apply to selected subplots OR the current axis
      ...
      for QTEMP4 = 1:length(QmatNMR.AllAxes)
        ...
        if ~isempty(QmatNMR.AxesNR)
          if QmatNMR.uiInput1a
            set(get(QmatNMR.AllAxes(QTEMP4), 'xlabel'), 'string', QTEMP1);
          end
          if QmatNMR.uiInput2a
            set(get(QmatNMR.AllAxes(QTEMP4), 'ylabel'), 'string', QTEMP2);
          end
          if QmatNMR.uiInput3a
            set(get(QmatNMR.AllAxes(QTEMP4), 'zlabel'), 'string', QTEMP3);
          end
        end	  
      end
      ...
      disp('Axis labels changed for selected axes ...');
    ...
    elseif (QmatNMR.uiInput4 == 2) 		%apply to all subplots
      ...
      disp('Axis labels changed for all axes ...');
    ...
    elseif (QmatNMR.uiInput4 == 3) 		%apply to the current row of subplots
      ...
      disp('Axis labels changed for all axes in the current row ...');
    ...
    elseif (QmatNMR.uiInput4 == 4) 		%apply to the current column of subplots
      ...
      disp('Axis labels changed for all axes in the current column ...');
    end  
  
  else
    disp('Setting of XLabel, YLabel and ZLabel properties was cancelled ...');
  end  
