...
%
%
%askCMXlaadSeries.m opens an input window to get information about the series of CMX processed data
...
try
  if (QmatNMR.Xfilename == 0)		%first check whether a previous filename exists
    CMXlaadSeries				%if not, go to the normal "binary FID" mode
    ...
    QuiInput('Read Chemagnetics spectra files from disk :', 'OK | CANCEL', 'regelCMXSpectrareadSeries', [], ...
             ...
             '&CKRead standard parameter files? &CAregelinputbuttonsCMXlaadSeries', QmatNMR.ReadParameterFilesFlag);
  end  
