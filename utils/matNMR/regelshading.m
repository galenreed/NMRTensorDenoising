...
%
%
%regelshading.m handles changing the shading property in the axis
%13-07-'99
...
try
  if QmatNMR.buttonList == 1		%OK-button
    ...
    if (QmatNMR.RecordingPlottingMacro)
      ...
      QmatNMR.Macro = AddToMacro(QmatNMR.Macro, 739, QTEMP12.SubPlots, QmatNMR.uiInput1, QmatNMR.uiInput2);
    end
    ...
    %Perform the action
    %
    QTEMP4 = str2mat('flat', 'faceted', 'interp');
    QTEMP1 = QTEMP4(QmatNMR.uiInput1, :);
    ...
    if (QmatNMR.uiInput2 == 1)		%only change selected axes
      ...
      disp(['Shading set to ' deblank(QTEMP1) ' for selected axes ...']);
    ...
    elseif (QmatNMR.uiInput2 == 2)		%change all axes in the current plot
      ...
      QmatNMR.AllAxes = findobj(findobj(QTEMP21, 'type', 'axes'), 'selected', 'on');
      if (length(QmatNMR.AllAxes) > 0)
        set(findobj(findobj(QTEMP21, 'type', 'axes'), 'selected', 'on'), 'selected', 'off');
        set(QmatNMR.AxisHandle2D3D, 'selected', 'on');
        %axes(QmatNMR.AxisHandle2D3D);
        set(QTEMP21, 'currentaxes', QmatNMR.AxisHandle2D3D);
      end
      ...
      disp(['Shading set to ' deblank(QTEMP1) ' for all axes ...']);
    ...
    elseif (QmatNMR.uiInput2 == 3)		%apply to the current row of subplots
      ...
      disp(['Shading set to ' deblank(QTEMP1) ' for all axes in the current row ...']);
    ...
    elseif (QmatNMR.uiInput2 == 4)		%apply to the current column of subplots
      ...
      disp(['Shading set to ' deblank(QTEMP1) ' for all axes in the current column ...']);
    end
  
  else
    disp('Shading not changed ...');
  end
