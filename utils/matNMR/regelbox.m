...
%
%
%regelbox.m handles changing whether or not to have a box in the axis
%13-07-'99
...
try
  if QmatNMR.buttonList == 1		%OK-button
    ...
    if (QmatNMR.RecordingPlottingMacro)
      ...
      QmatNMR.Macro = AddToMacro(QmatNMR.Macro, 729, QTEMP12.SubPlots, QmatNMR.uiInput1, QmatNMR.uiInput2);
    end
    ...
    if (QmatNMR.uiInput2 == 1)		%only change selected axes
      ...
      for QTEMP4 = 1:length(QmatNMR.AllAxes)
        ...
        if ~isempty(QmatNMR.AxesNR)
          set(QmatNMR.AllAxes(QTEMP4), 'box', QTEMP1);
        end	  
      end
      ...
      disp('Box setting changed for selected axes ...');
    ...
    elseif (QmatNMR.uiInput2 == 2) 		%apply to all subplots
      ...
      disp('Box setting changed for all axes ...');
    ...
    elseif (QmatNMR.uiInput2 == 3) 		%apply to the current row of subplots
      ...
      disp('Box setting changed for all axes in the current row ...');
    ...
    elseif (QmatNMR.uiInput2 == 4) 		%apply to the current column of subplots
      ...
      disp('Box setting changed for all axes in the current column ...');
    end  
  
  else
    disp('Box setting not changed ...');
  end
