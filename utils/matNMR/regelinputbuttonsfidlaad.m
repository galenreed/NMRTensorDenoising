...
%
%
% regelinputbuttonsfidlaad
%
% allows switching of the 'enable' parameter of the uicontrols in the input window
% when loading a binary FID. This is useful since for certain file formats the user
...
try
  ...
  %
  QTEMP1 = get(findobj(QmatNMR.uiInputFig, 'tag', '5'), 'value');
  
  %
  %The popups with the domain sizes
  %
  QTEMP2 = [findobj(QmatNMR.uiInputFig, 'tag', '2') findobj(QmatNMR.uiInputFig, 'tag', '3')];
  QTEMP3 = [findobj(QmatNMR.uiInputFig, 'tag', '2t') findobj(QmatNMR.uiInputFig, 'tag', '3t')];
  ...
  %
  QTEMP4 = [findobj(QmatNMR.uiInputFig, 'tag', '6')];
  QTEMP5 = [findobj(QmatNMR.uiInputFig, 'tag', '6t')];
  ...
  switch QTEMP1
    ...
    case 9		%JEOL Generic Data Format
      %
    ...
    case 10		%SMIS .mrd format
    %
    %For the SMIS format the parameters never need to be filled in
    %
      set(QTEMP2, 'enable' , 'off');
      set(QTEMP3, 'visible', 'off');
      set(QTEMP4, 'enable' , 'off');
    ...
    case 11		%CMXW fid?d format
      %
      %for the CMXW format it the parameter files may be read and so the time domain size buttons
      %may or may not be enabled, depending on whether the parameter files must be read
      %
      if ~get(QTEMP4, 'value')
        set(QTEMP2, 'enable' , 'on');
