...
%
%
function prt = T1pk_qvtdf(x,f,lam,dp,func)
...
for i=1:npeaks,
  ...
  prt(:,base+1) = lam(leng)*(exp (-x / lam(base+2)));
  prt(:,base+2) = (lam(base+1) .* prt(:, base+1) .* (x / (lam(base+2)^2)));
end
...
for i=1:npeaks
  ...
  prt(:, leng) = prt(:, leng) + (lam(base+1) .* exp(-x / lam(base+2)));
end
prt(:, leng) = prt(:, leng) + lam(leng-1);	%the background
