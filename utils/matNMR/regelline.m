...
%
%
%regelline.m takes care of the input for the plotting of 1D line plots in the 2D/3D
%viewer
%26-01-2004
...
try
  if QmatNMR.buttonList == 1
    ...
    QmatNMR.UserDefAxisT2Cont = QmatNMR.uiInput2;
    QmatNMR.ContourLineSpec = QmatNMR.uiInput3;
    QmatNMR.Q2D3DMacro = QmatNMR.uiInput4;
  ...
  %8 = line plot
  %
  QmatNMR.Q2D3DPlotType = 8;
  
  
  %
  %a 2D matrix has been supplied to the routine and this means a series of 1D spectra will be plotted
  %if possible
  %
    [QTEMP3 QTEMP4] = size(QmatNMR.Spec2D3D);
    if ((ndims(squeeze(QmatNMR.Spec2D3D)) == 2) & isempty(find([QTEMP3 QTEMP4] == 1)))
      disp('A 2D matrix has been supplied and will be plotted in the available subplots....');
      regelplotseries
      
    elseif (ndims(squeeze(QmatNMR.Spec2D3D)) > 2)
      error('matNMR ERROR: Matrix has a dimension size bigger than 2!');
  
    else
      QTEMP4 = length(QmatNMR.Spec2D3D);
