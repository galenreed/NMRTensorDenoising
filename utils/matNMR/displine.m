...
%
%
%displine.m is the actual plotting routine for line plots in the 2D/3D viewer
%26-01-2004
...
try
  ...
  %
    QmatNMR.Axis2D3DTD1 = QmatNMR.Axis2D3DTD2;	%to prevent CheckAxisCont from crashing if QmatNMR.Axis2D3DTD1 wasn't defined yet
    CheckAxisCont
    if length(QmatNMR.ContourLineSpec)
      QmatNMR.PlotString = ['plot(QmatNMR.Axis2D3DTD2Plot, QmatNMR.Spec2D3DPlot, QmatNMR.ContourLineSpec);'];
    else
      QmatNMR.PlotString = ['plot(QmatNMR.Axis2D3DTD2Plot, QmatNMR.Spec2D3DPlot, QmatNMR.LineColor);'];
    end
  ...
  %
    ...
    %
      ...
      set(QmatNMR.AxisHandle2D3D, 'xlim', [QmatNMR.Axis2D3DTD2Plot(1) QmatNMR.Axis2D3DTD2Plot(end)]);
      grid off;
