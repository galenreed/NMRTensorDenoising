...
%
%
% regelinputbuttonsimportMRISeries3
%
% also used instead of:
%
% regelinputbuttonsBrukerlaadSeries3
% regelinputbuttonsCMXlaadSeries3
% regelinputbuttonssimpsonasciilaadSeries2
%
...
try
  ...
  %
    ...
    %
    if (~isempty(QTEMP2) & (length(QTEMP2) == 1))
      %
      %find file separators around the $#$ code
      %
      QTEMP3 = sort([findstr(QTEMP1, filesep) findstr(QTEMP1, '.')]);
      ...
      elseif (QTEMP2 < QTEMP3(1)) 	%file separator found after '$#$' code
        QTEMP4 = QTEMP1(1:QTEMP3(1)-1);
      
      elseif (QTEMP2 > QTEMP3(end)) 	%last file separator found before '$#$ code
        QTEMP4 = QTEMP1(QTEMP3(end)+1:end);
        
        if (strcmp(QTEMP4, '$#$') & length(QTEMP3 > 1))
          QTEMP4 = [QTEMP1(QTEMP3(end-1)+1:QTEMP3(end)-1) '_' QTEMP4];
        end
      
      else 				%'$#$ code sandwiched between two file separators
        QTEMP3 = sort([QTEMP3 QTEMP2]);
        QTEMP4 = QTEMP1( QTEMP3(find(QTEMP3 == QTEMP2) - 1)+1:QTEMP3(find(QTEMP3 == QTEMP2) + 1)-1);
        
        if (strcmp(QTEMP4, '$#$') & (find(QTEMP3 == QTEMP2) > 1))
          QTEMP4 = [QTEMP1( QTEMP3(find(QTEMP3 == QTEMP2) - 2)+1:QTEMP3(find(QTEMP3 == QTEMP2) - 1)-1) '_' QTEMP4];
        end
      end
      ...
      %
      set(findobj(QmatNMR.uiInputFig, 'tag', '3'), 'string', QTEMP4);
    end
