...
%
%
%regelaxis.m handles changing of the axis properties using the "axis" command
%13-07-'99
...
try
  if QmatNMR.buttonList == 1		%OK-button
    ...
    if (QmatNMR.RecordingPlottingMacro)
      ...
      QmatNMR.Macro = AddToMacro(QmatNMR.Macro, 721, QTEMP12.SubPlots, QmatNMR.uiInput1, QmatNMR.uiInput2, get(QTEMP20, 'userdata'));
    end
    ...
    %Perform the action
    %
    QTEMP1 = str2mat('on', 'off', 'auto', 'manual', 'square', 'normal', 'tight', 'fill', 'ij', 'xy', 'equal', 'image');
    ...
    if (QmatNMR.uiInput2 == 1)		%only change selected axes
      ...
      for QTEMP3 = 1:length(QmatNMR.AllAxes)
        ...
        if ~isempty(QmatNMR.AxesNR)
          %axes(QmatNMR.AllAxes(QTEMP3));
          set(QTEMP21, 'currentaxes', QmatNMR.AllAxes(QTEMP3));
      ...
      disp('Axis scaling and appearance changed for selected axes ...');
    ...
    elseif (QmatNMR.uiInput2 == 2)		%change all axes in the current plot
      ...
      disp('Axis scaling and appearance changed for all axes in the current row ...');
  
  
    elseif (QmatNMR.uiInput2 == 3)		%change all axes in the current row of subplots to the same color axis
      QmatNMR.AllAxes = findobj(findobj(QTEMP21, 'type', 'axes', 'tag', 'RegularAxis'), 'selected', 'on');
      if isempty(QmatNMR.AllAxes)
        QmatNMR.AllAxes = QTEMP20;
      end
      ...
      disp('Axis scaling and appearance changed for all axes ...');
  
  
    elseif (QmatNMR.uiInput2 == 4)		%change all axes in the current column of subplots to the same color axis
      QmatNMR.AllAxes = findobj(findobj(QTEMP21, 'type', 'axes', 'tag', 'RegularAxis'), 'selected', 'on');
      if isempty(QmatNMR.AllAxes)
        QmatNMR.AllAxes = QTEMP20;
      end
      ...
      disp('Axis scaling and appearance changed for all axes in the current column ...');
    end
  
  else
    disp('Axis scaling and appearance not changed ...');
  end
