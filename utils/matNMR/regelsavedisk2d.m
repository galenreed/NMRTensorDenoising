...
%
%
%regelsavedisk2d.m takes care of saving the current 2D spectrum to disk
%21-12-'96
...
try
  if QmatNMR.buttonList == 1						%= OK-button
    ...
    QmatNMR.SaveHypercomplex = QmatNMR.uiInput6;
    QmatNMR.SaveAxis = QmatNMR.uiInput7;
    ...
    if isempty(findstr(QmatNMR.uiInput1, ' '))		%don't do following when multiple variables have been entered
      if (QmatNMR.SaveHistory | QmatNMR.SaveHypercomplex | QmatNMR.SaveAxis)	%Connect history and/or hypercomplex part to spectrum --> make structure
        ...
        disp(['The current 2D spectrum was saved as a structure on disk as : "' QmatNMR.uiInput2 '"']);
      ...
      eval([QmatNMR.uiInput1 '.PeakListText = [];']);
        ...
        disp(['The current 2D spectrum was saved as a matrix on disk as : "' QmatNMR.uiInput2 '"']);
      end
    else
      disp(['The variables have been saved on disk as : "' QmatNMR.uiInput2 '"']);
    end
  
  						%Now save the spectrum to disk
    ...
    if (QmatNMR.uiInput8) 				%if needed clear variable from the workspace
